...
type Bawang struct {
	...
	ID string `json:"id"`
	BenihGenesisID string `json:"benihGenesisID"`
	BawangGenesisID string `json:"bawangGenesisID"`
...
func (s *BawangContract) CreateBenih(ctx contractapi.TransactionContextInterface, bawangData string) (string, error) {
	...
	// Set GenesisID
	bawang.BenihGenesisID = ctx.GetStub().GetTxID()
...
func (s *BawangContract) CreateTrxBawangByPenangkar(ctx contractapi.TransactionContextInterface, bawangData, prevID string) (string, error) {
	...
	if len(prevID) == 0 {
		return "", fmt.Errorf("Please pass the correct benih genesis id")
	}

	bawangPrev, err := s.GetBawangByID(ctx, prevID) 

	if err != nil {
		return "", fmt.Errorf("Failed while getting benih genesis. %s", err.Error())
	}
	...
	// Set GenesisIDs
	bawangNew.BenihGenesisID = bawangPrev.BenihGenesisID

	bawangNew.IsGenesis = false
	bawangNew.IsConfirmed = false

	// Get penangkar unique value from benih genesis
	...
	if bawangPrev.KuantitasBenihKg - bawangNew.KuantitasBenihKg >= 0 {
		...
		if err != nil {
			return "", fmt.Errorf("Failed while updating benih genesis kuantitas. %s", err.Error())
		}
...
func (s *BawangContract) CreateBawang(ctx contractapi.TransactionContextInterface, bawangData, prevID string) (string, error) {
	...
	// Set GenesisID
	bawangNew.BawangGenesisID = ctx.GetStub().GetTxID()
	bawangNew.BenihGenesisID = bawangPrev.BenihGenesisID

	bawangNew.TxID1 = bawangPrev.TxID1

	bawangNew.IsGenesis = true
	bawangNew.IsConfirmed = false
...
func (s *BawangContract) UpdateBawangTrxByPetani(ctx contractapi.TransactionContextInterface, bawangData, prevID string) (string, error) {
	...
	if len(prevID) == 0 {
		return "", fmt.Errorf("Please pass the correct bawang genesis id")
	}

	bawangPrev, err := s.GetBawangByID(ctx, prevID) 

	if err != nil {
		return "", fmt.Errorf("Failed while getting bawang genesis. %s", err.Error())
	}
	...
	// Set GenesisID
	bawangNew.BawangGenesisID = bawangPrev.BawangGenesisID
	bawangNew.BenihGenesisID = bawangPrev.BenihGenesisID
	...
	// Get penangkar unique field from bawang genesis
	bawangNew.UmurBenih = bawangPrev.UmurBenih
	bawangNew.UmurPanen = bawangPrev.UmurPanen
	bawangNew.LamaPenyimpanan = bawangPrev.LamaPenyimpanan
	bawangNew.Varietas = bawangPrev.Varietas
	...
	if bawangPrev.KuantitasBawangKg - bawangNew.KuantitasBawangKg >= 0 {
		...
		if err != nil {
			return "", fmt.Errorf("Failed while updating bawang genesis kuantitas. %s", err.Error())
		}
...
func (s *BawangContract) checkUserbyUsername(ctx contractapi.TransactionContextInterface, username string) (string, error) {
	...
	res, err := s.GetBawangForQuery(ctx, `{\"selector\":{\"username\":\"`+ username +`\"}}`)
	...
	if orgName == "Penangkar" || orgName == "Petani" {
		return result["result"][0]["Record"]["alamatLahan"].(string), nil
	} else {
		return result["result"][0]["Record"]["alamatToko"].(string), nil
	}
...
func (s *BawangContract) updateKuantitasBenihByID(ctx contractapi.TransactionContextInterface, bawangID string, kuantitasBenihNext float64) (string, error) {
	if len(bawangID) == 0 {
		return "", fmt.Errorf("Please insert the correct benih genesis id")
	}
	...
	if bawangAsBytes == nil {
		return "", fmt.Errorf("Benih genesis with %s id does not exist", bawangID)
	}
...
func (s *BawangContract) updateKuantitasBawangByID(ctx contractapi.TransactionContextInterface, bawangID string, kuantitasBawangNext float64) (string, error) {
	if len(bawangID) == 0 {
		return "", fmt.Errorf("Please insert the correct bawang genesis id")
	}
	...
	if bawangAsBytes == nil {
		return "", fmt.Errorf("Bawang genesis with %s id does not exist", bawangID)
	}
